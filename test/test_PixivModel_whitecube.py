...
#        p = open('./test_data/work_details_modal_whitecube.json', 'r')
...
#        p = open('./test_data/work_details_modal_whitecube-manga.json', 'r')
...
#        self.assertEqual(image.imageMode, "manga")
    ...
    def testParseMemberError(self):
        p = open('./test_data/ajax-error.json', 'r')
        try:
    ...
    def testParseMemberImages(self):
        p = open('./test_data/all-14095911.json', 'r')
        member = PixivArtist(14095911, p.read(), False, 0, 24)
        self.assertIsNotNone(member)
        p2 = open('./test_data/userdetail-14095911.json', 'r')
    ...
    def testParseMemberImages2(self):
        p = open('./test_data/all-26357.json', 'r')
        member = PixivArtist(26357, p.read(), False, 0, 24)
        self.assertIsNotNone(member)
        p2 = open('./test_data/userdetail-26357.json', 'r')
        info = json.loads(p2.read())
        member.ParseInfo(info, False, False)
    ...
    def testParseMemberImagesByTags(self):
        p = open('./test_data/tag-R-18-14095911.json', 'r')
        member = PixivArtist(14095911, p.read(), False, 0, 24)
        self.assertIsNotNone(member)
        p2 = open('./test_data/userdetail-14095911.json', 'r')
        info = json.loads(p2.read())
    ...
    def testParseMemberImagesByTagsLastPage(self):
        p = open('./test_data/tag-R-18-14095911-lastpage.json', 'r')
    ...
    def testParseMemberBookmarksByTags(self):
        p = open('./test_data/bookmarks-1039353.json', 'r')
        member = PixivArtist(1039353, p.read(), False, 0, 24)
        self.assertIsNotNone(member)
        p2 = open('./test_data/userdetail-1039353.json', 'r')
        info = json.loads(p2.read())
